%% CA3 MPPCA_Mahal angle for summary
%% load mat-file
clc; clear; close all;
...
%%
% A: CS cell, B: US cell, C: LTM-CS cell
...
thrcov_PC_percent = 70 ;% not working
reference_session_num = 5 ; % 2:base, 3:CS, 4:US, 5:ITI1, 6:ITI2, 7:LTM-base, 8:LTM-CS
num_top_PCs = 3 ; % 3 off
...
[res_MPPCA_data_cell{i,1}, res_Angle_data_cell{i,1}, res_Mahald_data_cell{i,1}, res_CS_cells_data_cell{i,1}] = ...
    fxn_CA3_MP_PCA_Mahalanobis_summary(data_temp, thrcov_PC_percent, reference_session_num, num_top_PCs);
end
% out_table{i,1}  = 
toc;
%%
...
Mahald_mean_A = cell2mat(Mahald_mean_A); % for CS
Mahald_mean_B = cell2mat(Mahald_mean_B); % for US
...
Angle_mean_A = cell2mat(Angle_mean_A); % for CS
Angle_mean_B = cell2mat(Angle_mean_B); % for US
...
results_PVD_CS_ITI_percent = (CS_PVD_temp-1)*100; % for fig6i PVD

US_PVD_temp           = Mahald_mean_B(:,[4,3]);
US_PVD_temp           = US_PVD_temp ./ US_PVD_temp(:,1);
results_PVD_US_ITI_percent = (US_PVD_temp-1)*100; % for fig6j PVD
%% ### for figure, Relative rotation cal between CS or US and ITI-1 ###
results_PVD_CS_ITI_angle = Angle_mean_A(:,[4,2]); % for fig6i CS angle
results_PVD_US_ITI_angle = Angle_mean_B(:,[4,3]); % for fig6j US angle
