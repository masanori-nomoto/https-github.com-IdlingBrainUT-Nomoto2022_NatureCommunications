function fxn_boxplot_double(G1_results_ratio, G2_results_ratio, input_session_num, section_num)
%%
%%
if input_session_num == 6
% section_num = 2;
Box_X = [];
Box_Y = [];
...
%% Boxplot new
% figure('color',[1,1,1]);
figure('Position',[244,181,150,130]); %[left bottom width height]
boxplot(Box_X, Box_Y,'factorgap',9.2,'color','kr','OutlierSize',2, 'Widths' ,1.5,'Symbol','','BoxStyle','outline','Notch','off')

% axis([0 40 0.004 30]); % for filtered only data
axis([-0.5 25 -0.05 0.75]); % for filtered only data
% set(gca,'YScale','log');
set(gca,'xtick', 2: 4 : 25)
set(gca,'xticklabel',{'Baseline', 'CS','US','ITI-1','ITI-2','Rest'})
xtickangle(90);
...
% axis([0 40 0.004 30]); % for filtered only data
axis([0 25 -0.05 0.6]); % for filtered only data, CS LTMCS 0.6, US 1.1
% axis([0 25 -0.05 1.25]); % for US filtered only data
% set(gca,'YScale','log');
