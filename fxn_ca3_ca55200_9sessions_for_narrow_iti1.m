function [results] = fxn_ca3_ca55200_9sessions_for_narrow_iti1(upsample_rate)
%% for debug,
...
% t.s10 = [2761:2880]; % LTM baseline
% t.s11 = [2881:2910 2941:2970 3001:3030 3061:3090]; % LTM CS
% t.s12 = [2911:2940 2971:3000 3031:3060 3091:3120]; % LTM ITI
...
% 220817 edite for additional analysis
% ITI-1 default 165-180 
% t_temp_s4 = [165*upsample_rate+1 + 45*upsample_rate*(i-1): 165*upsample_rate + 45*upsample_rate*(i-1) + 15*upsample_rate];

% ITI-1 narrow  def=165; 170-180: p=0.08; 171-180: p=0.05; 169-179: P=0.0535
% t_temp_s4 = [165 *upsample_rate+1 + 45*upsample_rate*(i-1): 165*upsample_rate + 45*upsample_rate*(i-1) + 5 *upsample_rate]; % 1-5 s in ITI-1
t_temp_s4 = [170 *upsample_rate+1 + 45*upsample_rate*(i-1): 170*upsample_rate + 45*upsample_rate*(i-1) + 9 *upsample_rate]; % 6-14 s in ITI-1
t.s4 = cat(2,t.s4,t_temp_s4); % ITI-E

t_temp_s5 = [180*upsample_rate+1 + 45*upsample_rate*(i-1): 180*upsample_rate + 45*upsample_rate*(i-1) + 15*upsample_rate]; 
t.s5 = cat(2,t.s5,t_temp_s5); % ITI-L
end
