%% CA3 MP-PCA Mahalanobis distance analysis (210609 revised)
function [res_MPPCA_data_cell, res_Angle_data_cell, res_Mahald_data_cell, res_CS_cells_data_cell]  = ...
    fxn_CA3_MP_PCA_Mahalanobis_summary(input_data, thrcov_PC_percent, reference_session_num, num_top_PCs)
...
% % % 220820 addeditional analysis for pure CS US LTM-CS ensemble
% MPPCA_data_cell{6,2}  = Cell_type_ABC_pure_A_ID ; % PCA_cellIDs_ABC
% MPPCA_data_cell{7,2} = Cell_type_ABC_pure_B_ID ; % PCA_cellIDs_ABC
% MPPCA_data_cell{8,2} = Cell_type_ABC_pure_C_ID ; % PCA_cellIDs_ABC

% % % for error-escaping for PVD cal
% if numel(Cell_type_ABC_pure_C_ID)==1
% Cell_type_ABC_pure_C_ID = [66 67 68 69 70]
% MPPCA_data_cell{8,2} = Cell_type_ABC_pure_C_ID ; % PCA_cellIDs_ABC
% end
%% MP-PCA cal
%  [res_MPPCA, res_thrcov_PCA] = fxn_Marchenko_thrcover_PCA(ca_data, thrcov_PC_percent)
% thrcov_PC_percent = 90;
MPPCA_analysis_range = [1:600, 2401:2760];
...
Mahald_data_cell{2,7} = time_stamp_info{7,1}-1800;
Mahald_data_cell{2,8} = time_stamp_info{8,1}-1800;
Mahald_data_cell{2,9} = time_stamp_info{9,1}-1800;
...
% debug
% figure; 
% subplot(211); plot(MPPCA_data_cell{ii, 5}.thrcov_PCA_score(Mahald_data_cell{2,reference_session_num},:));
% subplot(212); plot(MPPCA_data_cell{ii, 5}.thrcov_PCA_score(Mahald_data_cell{2,i},:));

% % normal cal using thresholded all PCs
...
%      MPPCA_data_cell{ii, 5}.thrcov_PCA_score(Mahald_data_cell{2,i},:));    % target
 ...
 % for dimension reduction, take top 5 PCs.
%  num_top_PCs = 10;
%  [Angle_data_cell{ii+1,i}, Angle_data_cell{ii+9,i}] = fxn_nmt_cosine...
%     (MPPCA_data_cell{ii, 5}.thrcov_PCA_score(Mahald_data_cell{2,reference_session_num},(1:num_top_PCs)), ... % reference
%      MPPCA_data_cell{ii, 5}.thrcov_PCA_score(Mahald_data_cell{2,i},(1:num_top_PCs)));    % target
end
end
%% compare CS and ITI
